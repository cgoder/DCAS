...
//#define CDCA_MAXLEN_CARDREADER 32 // 读取器名称的最大长度
#define CDCA_MAXNUM_CARD 4 // 同时插入卡的最大个数
#define CDCA_MAXNUM_CY_FILE 4//最多同时打开的加扰文件个数
...
#define CDCA_RC_CARDHANDLE_INVALID    0x13  /* 卡句柄无效 */
...
/*-- 读卡器中的智能卡状态 --*/
/*-- 读卡器中的智能卡状态 按优先级排列--*/
#define CDCA_SC_IN          0x00    /* 读卡器中是可用的卡      */
#define CDCA_SC_UPDATE      0x01    /* 读卡器的卡升级没有完成  */
#define CDCA_SC_UPDATE_ERR  0x02    /* 读卡器的卡升级失败      */
#define CDCA_SC_ERROR       0x03    /* 读卡器的卡不能识别      */
#define CDCA_SC_INSERTING   0x04    /* 正在插卡，初始化        */
#define CDCA_SC_REMOVING    0x05    /* 正在拔卡，重置状态      */
#define CDCA_SC_OUT         0x06    /* 读卡器中没有卡          */
...
/* 数据限制定义 */
#define CDCA_MAXNUM_DECFILE   4  /* 同时处理文件的最大个数 */
/* 错误代码定义 */
#define CDCA_ERRORNO_NONE                0x0000  /* 无错误 */
#define CDCA_ERRORNO_UNKNOWN             0x0001  /* 未知错误，如分配内存错误等 */
#define CDCA_ERRORNO_CARD_INVALID        0x0002  /* 智能卡无效，如未插卡、无法识别卡、智能卡通讯错误等 */
#define CDCA_ERRORNO_NOENTITLE           0x0003  /* 无授权 */
#define CDCA_ERRORNO_NOROOM              0x0004  /* 没有空间，已达解扰最大记录个数 */
#define CDCA_ERRORNO_FILEOPERATE         0x0005  /* 文件操作错误，如open、read等操作；可以通过标准库的errno分析 */
#define CDCA_ERRORNO_NOTSUPPORTDEC       0x0006  /* DRM终端不支持此解密算法 */
...
/*-- IPPV/IPPT节目购买提示信息 --*/
typedef struct {
    CDCA_U8         m_scNum;				/*支持的卡的个数*/
	void*           m_scHandle[CDCA_MAXNUM_CARD];             /* 使用的卡句柄 */
...
/* 插入智能卡 */
extern CDCA_BOOL CDCASTB_SCInsert(void * scHandle);

/* 拔出智能卡*/
extern void CDCASTB_SCRemove( void * scHandle );

/* 读取智能卡外部卡号 */
extern CDCA_U16 CDCASTB_GetCardSN(void * scHandle, char* pCardSN );

/* PIN码管理 */
extern CDCA_U16 CDCASTB_ChangePin(void * scHandle, const CDCA_U8* pbyOldPin,
                                   const CDCA_U8* pbyNewPin);

/* 设置用户观看级别 */
extern CDCA_U16 CDCASTB_SetRating(void * scHandle, const CDCA_U8* pbyPin,
                                   CDCA_U8 byRating );

/* 查询用户观看级别 */
extern CDCA_U16 CDCASTB_GetRating(void * scHandle, CDCA_U8* pbyRating );

/* 设置智能卡工作时段 */
extern CDCA_U16 CDCASTB_SetWorkTime(void * scHandle, const CDCA_U8* pbyPin,
									 CDCA_U8        byStartHour,
									 CDCA_U8        byStartMin,
									 CDCA_U8        byStartSec,
									 CDCA_U8        byEndHour,
...
/* 查询智能卡当前工作时段 */
extern CDCA_U16 CDCASTB_GetWorkTime(void * scHandle, CDCA_U8* pbyStartHour,
									 CDCA_U8* pbyStartMin,
									 CDCA_U8* pbyStartSec,
									 CDCA_U8* pbyEndHour,
									 CDCA_U8* pbyEndMin,
...
/* 查询运营商ID列表 */
extern CDCA_U16 CDCASTB_GetOperatorIds(void * scHandle, CDCA_U16* pwTVSID );

/* 查询运营商信息 */
extern CDCA_U16 CDCASTB_GetOperatorInfo(void * scHandle,  CDCA_U16           wTVSID,
                                         SCDCAOperatorInfo* pOperatorInfo );

/* 查询用户特征 */
extern CDCA_U16 CDCASTB_GetACList( void * scHandle, CDCA_U16 wTVSID, CDCA_U32* pACArray );

/* 查询钱包ID列表 */
extern CDCA_U16 CDCASTB_GetSlotIDs(void * scHandle, CDCA_U16 wTVSID, CDCA_U8* pbySlotID);

/* 查询钱包的详细信息 */
extern CDCA_U16 CDCASTB_GetSlotInfo(void * scHandle,  CDCA_U16 wTVSID,
                                     CDCA_U8           bySlotID,
                                     SCDCATVSSlotInfo* pSlotInfo );

/* 查询普通授权节目购买情况 */
extern CDCA_U16 CDCASTB_GetServiceEntitles(void * scHandle, CDCA_U16       wTVSID,
                                            SCDCAEntitles* pServiceEntitles );


 // add by zhangchao 2010-12-7: 读取高级预览记录，内部测试用
/*-- 高级预览节目购买记录查询 --*/
/*
参数说明：
	wTvsID	：运营商ID
...
*/
extern CDCA_U16 CDCASTB_GetCurtainRecords(void * scHandle,  CDCA_U16 wTvsID,
...
/* 查询授权ID列表 */
extern CDCA_U16 CDCASTB_GetEntitleIDs(void * scHandle,  CDCA_U16  wTVSID,
                                       CDCA_U32* pdwEntitleIds );


/* 查询反授权确认码 */
extern CDCA_U16 CDCASTB_GetDetitleChkNums(void * scHandle,  CDCA_U16   wTvsID,
                                           CDCA_BOOL* bReadFlag,
                                           CDCA_U32*  pdwDetitleChkNums);

/* 查询反授权信息读取状态 */
extern CDCA_BOOL CDCASTB_GetDetitleReaded(void * scHandle,  CDCA_U16 wTvsID );

/* 删除反授权确认码 */
extern CDCA_BOOL CDCASTB_DelDetitleChkNum(void * scHandle,  CDCA_U16 wTvsID,
...
/* 查询机卡对应情况 */
extern CDCA_U16 CDCASTB_IsPaired(void * scHandle,  CDCA_U8* pbyNum,
...
/* IPPV节目购买 */
extern CDCA_U16 CDCASTB_StopIPPVBuyDlg(void * scHandle,  CDCA_BOOL       bBuyProgram,
                                        CDCA_U16        wEcmPid,
                                        const CDCA_U8*  pbyPinCode,
                                        const SCDCAIPPVPrice* pPrice );

/* IPPV节目购买情况查询 */
extern CDCA_U16 CDCASTB_GetIPPVProgram(void * scHandle,  CDCA_U16       wTvsID,
                                        SCDCAIppvInfo* pIppv,
                                        CDCA_U16*      pwNumber );
...
/* 读取子母卡信息 */
extern CDCA_U16 CDCASTB_GetOperatorChildStatus(void * scHandle,  CDCA_U16   wTVSID,
                                                CDCA_U8*   pbyIsChild,
...
/* 读取母卡喂养数据 */
extern CDCA_U16 CDCASTB_ReadFeedDataFromParent(void * scHandle,  CDCA_U16 wTVSID,
                                                CDCA_U8* pbyFeedData,
                                                CDCA_U8* pbyLen     );

/* 喂养数据写入子卡 */
extern CDCA_U16 CDCASTB_WriteFeedDataToChild(void * scHandle,  CDCA_U16       wTVSID,
...
/* 智能卡复位 */
extern CDCA_BOOL CDSTBCA_SCReset(void * scHandle,  CDCA_U8* pbyATR, CDCA_U8* pbyLen );

/* 智能卡通讯 */
extern CDCA_BOOL CDSTBCA_SCPBRun(void * scHandle,  const CDCA_U8* pbyCommand, 
...
/* 通知授权变化 */
extern void CDSTBCA_EntitleChanged(void * scHandle,  CDCA_U16 wTvsID );
...
/* 进度显示 */
//byMark == 1 时 scHandle无效
extern void CDSTBCA_ShowProgressStrip(void * scHandle,  CDCA_U8 byProgress,
                                       CDCA_U8 byMark );
...
extern int CDSTBCA_FS_open(const char *pathname,int  flages);

extern int CDSTBCA_FS_close(int fd);

extern size_t CDSTBCA_FS_read(int fd,void *buf,size_t count);

extern int CDSTBCA_FS_lseek(int fd,int offset,int whence);
 ...
 *   CA_LIB的版本号。
 */    
void CDCASTB_CY_GetVer(char pDrmVer[32]);


/*----------------------------------------------------------------------------*/
/*** 文件管理，以下接口的参数和返回值参考POSIX标准库 ***/
 ...
 *   !0：非超影加密的文件
*/
int CDCASTB_CY_CheckFile(const char *pathname, int flags);


/* 打开文件 */
int CDCASTB_CY_open(const char *pathname, int flags);


/* 关闭文件 */
int CDCASTB_CY_close(int fd);


/* 读取文件 */
size_t CDCASTB_CY_read(int fd, void *buf, size_t count);


/* 文件定位 */
size_t CDCASTB_CY_lseek(int fd, size_t offset, int whence);
 ...
 *   无
*/
void CDCASTB_CY_GetErrorCode(int *ErrCode);
